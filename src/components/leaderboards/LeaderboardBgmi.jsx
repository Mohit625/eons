// src/components/leaderboards/LeaderboardBgmi.jsx
...
// Removed: BGMI-Specific Types and Constants
const BGMI_MATCH_KEYS = ["match1", "match2", "match3", "match4", "match5", "match6"]; // Removed: as const
// Removed: BGMI_MatchKey type
// Removed: MatchRow type
// Removed: GroupKey type
// Removed: FinalsRow type
// Removed: LeaderboardBgmiProps type

const LeaderboardBgmi = ({ eventId, game, canEdit }) => { 
const GROUPS = (eventId === "lock-load") ? { A: [0, 7], B: [8, 15], C: [16, 23], D: [24, 30] } : { A: [0, 8], B: [9, 17], C: [18, 26], D: [27, 35] } ; // Removed: as const
  ...
  const initialRows = useMemo(() => teamNames.map((t) => ({ team: t, wwcd: 0, placement: 0, kills: 0, total: 0 })), [teamNames]); // Removed: : MatchRow[]
  ...
  const createMatchTemplate = useCallback(
    () =>
      BGMI_MATCH_KEYS.reduce((acc, key) => {
        ...
        return acc;
      }, {}), // Removed: as Record<BGMI_MatchKey, MatchRow[]>
    [initialRows]
  );

  const [matchData, setMatchData] = useState(() => createMatchTemplate()); // Removed: <Record<BGMI_MatchKey, MatchRow[]>>
  const [finalsRows, setFinalsRows] = useState([]); // Removed: <FinalsRow[]>
  // --- REMOVED: nziggerStats state ---

  // BGMI Logic
  const getTeamName = useCallback((origIdx) => { // Removed: : number
    const firstMatch = matchData[BGMI_MATCH_KEYS[0]];
    return firstMatch && firstMatch[origIdx] ? firstMatch[origIdx].team : `Team ${origIdx + 1}`;
  }, [matchData]);
  
  const updateTeamName = (origIdx, name) => { // Removed: : number, : string
    setMatchData((prev) => {
      const next = { ...prev }; // Removed: : typeof prev
  ...
  const updateCell = (m, i, key, value) => { // Removed parameter types
    setMatchData((prev) => {
      const next = { ...prev };
      const rows = [...next[m]];
      const row = { ...rows[i] };
      if (key === "wwcd" || key === "placement" || key === "kills") {
        row[key] = Number(value) || 0; // Removed: (row as any)
  ...
  const indicesForGroups = (...groups) => // Removed: : GroupKey[]
    groups.flatMap((g) => {
      const [s, e] = GROUPS[g];
      return Array.from({ length: e - s + 1 }, (_, i) => s + i);
    });

  const indicesForMatch = (mk) => { // Removed: : BGMI_MatchKey, : number[]
    switch (mk) {
  ...
  const groupLetterForIndex = (idx) => { // Removed: : number, return type
  ...
  const overallTotalsByIndex = useMemo(() => {
    const map = new Map(); // Removed: <number, number>
  ...
  const updateFinalsCell = (origIdx, key, value) => { // Removed parameter types
    const numericValue = Number(value) || 0;
    // --- REMOVED: if (origIdx === -1) block ---
    setFinalsRows((prev) =>
      prev.map((row) => {
        if (row.origIdx !== origIdx) return row;
        const updated = { ...row, [key]: numericValue }; // Removed: as FinalsRow
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const snapshot = await fetchPointsSnapshot(eventId, game.id); // Removed: <BGMI_MatchKey>
        if (cancelled) return;
        if (!snapshot) {
        ...
        if (snapshot.finals && snapshot.finals.length > 0) {
          ...
          // --- REMOVED: nziggerRow logic ---
          setFinalsRows(
            // --- MODIFIED: Filter logic simplified ---
            finalsList.filter((r) => r.origIdx !== -1 && r.team !== 'N-ZIGGER').slice(0, 16).map((row) => { // Removed: (r: any), (row: any)
          ...
          // --- REMOVED: if (nziggerRow) block ---
        } else {
          setFinalsRows([]);
          // --- REMOVED: setNziggerStats call ---
        }
  ...
  const saveBgmiPoints = async () => {
    try {
      ...
      const payload = {
        matchData,
        // --- MODIFIED: finals array no longer includes N-ZIGGER ---
        finals: [
        ...
        <TabsContent value="knockout">
          <div>
            <Tabs defaultValue="group-a">
              ...
              {(['A', 'B', 'C', 'D']).map((letter) => ( // Removed: as GroupKey[]
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {/* --- MODIFIED: Removed the hardcoded N-ZIGGER row --- */}
